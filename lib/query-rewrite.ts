...
// 한자(CJK Unified) + 일본어(히라가나·가타카나) 감지 패턴
const NON_KOREAN_RE = /[぀-ヿ一-鿿]/
...
export async function rewriteQuery(question: string, modelId: string): Promise<string> {
  ...
  const { text } = await generateText({
    ...
    system: '당신은 한국어 전용 금융 검색 도우미입니다. 반드시 한국어로만 답변하십시오. 한자, 중국어, 일본어는 절대 사용하지 마십시오.',
...
재작성:`,
  ...
  // 한자·일본어 혼입 감지 시 원문 반환
  if (!rewritten || NON_KOREAN_RE.test(rewritten)) return question
