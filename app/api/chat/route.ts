...
export async function POST(req: Request) {
  ...
  logger.info({
    ...
    contextSize: retrievedContext ? Buffer.byteLength(retrievedContext, 'utf8') : 0,
...
${retrievedContext?.trim() || '업로드된 문서가 없습니다. 문서를 업로드한 후 질문해 주세요.'}
  ...
  // 스트리밍이 완료된 후 LangSmith에 run 기록 (fire-and-forget)
  const userQuestion = messages?.findLast((m: { role: string }) => m.role === 'user')?.content ?? ''
  const traceRun = traceable(
    async (_input: { question: string; modelId: string; contextSize: number }) => {
      const text = await result.text
      return text
    },
    { name: 'fin-mate-chat', run_type: 'llm' }
  )
  traceRun({
    question: userQuestion,
    modelId: modelId ?? 'local-model',
    contextSize: retrievedContext ? Buffer.byteLength(retrievedContext, 'utf8') : 0,
  }).catch((err) => logger.warn({ event: 'langsmith_trace_failed', err: String(err) }, 'LangSmith 트레이스 실패'))
