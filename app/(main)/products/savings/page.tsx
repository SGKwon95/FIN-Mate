...
export const metadata: Metadata = { title: "적금" }
...
export default async function SavingsProductsPage() {
  const products = await prisma.product.findMany({
    where: {
      ...
      depositDetail: { transactionType: "SAVINGS" },
    },
    ...
    select: {
      ...
      depositDetail: {
        select: { minPeriodMonths: true, maxPeriodMonths: true, minAmount: true, maxAmount: true },
      },
      productRates: {
        where: { rateType: "BASE" },
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      <h1 className="text-lg font-bold text-kb-navy mb-4">적금 ({products.length})</h1>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {p.depositDetail?.minAmount != null && (
                    <p className="text-[11px] text-kb-gray mt-1">
                      월 {formatKRW(Number(p.depositDetail.minAmount))}
                      {p.depositDetail.maxAmount != null &&
                        ` ~ ${formatKRW(Number(p.depositDetail.maxAmount))}`}
                    </p>
                  )}
                  {p.isDepositInsured && (
      ...
      <ChatPopup productContext="현재 페이지: 적금 상품 목록" />
    </div>
  )
}
